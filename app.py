...
def get_balance_changes(meta, address):
    """Extract balance changes from transaction metadata with corrected holdings logic."""
    changes = {'XRP': 0}
        ...
        if 'ModifiedNode' in node:
            ...
            if modified.get('LedgerEntryType') == 'AccountRoot' and 'PreviousFields' in modified:
                ...
                if 'Balance' in previous_fields:
                    ...
                    if high == address or low == address:
                        ...
                        # Corrected: holdings change = -delta if low, else delta
            ...
            if created.get('LedgerEntryType') == 'RippleState':
                ...
                if high == address or low == address:
                    ...
                    # Corrected: initial holdings = -balance if low, else balance
            ...
            if deleted.get('LedgerEntryType') == 'RippleState':
                ...
                if high == address or low == address:
                    ...
                    # Corrected: holdings change = balance if low, else -balance
                    changes[token_key] = changes.get(token_key, 0) + (balance if low == address else -balance)
    return changes

def get_current_price(currency, issuer, transactions):
    """Fetch current token price in XRP with improved fallbacks."""
        ...
        try:
            ...
            for pair in response.get('pairs', []):
                if (pair.get('chainId') == 'xrpl' and 
                    ...
                    logger.debug(f"DEX Screener price for {decoded_currency}-{issuer}: {price}")
                    return price
            logger.warning(f"No DEX Screener price found for {decoded_currency}-{issuer}")
            return None
        except Exception as e:
    ...
    def get_dex_price():
        try:
            ...
            if buy_price and sell_price:
                ...
                logger.debug(f"DEX price for {currency}-{issuer}: {price}")
                return price
            price = buy_price or sell_price
            if price:
                logger.debug(f"DEX price (single side) for {currency}-{issuer}: {price}")
                return price
            logger.warning(f"Insufficient DEX data for {currency}-{issuer}")
            return None
        except Exception as e:
    ...
    def get_historical_price():
        ...
        if price:
            logger.debug(f"Historical price for {currency}-{issuer}: {price}")
        return price

    # Prioritize DEX price, then DEX Screener, then historical
    for method in (get_dex_price, get_dexscreener_price, get_historical_price):
        price = method()
        if price and price > 0:
            return price
    logger.warning(f"Using fallback price 0.000001 for {currency}-{issuer}")
    return 0.000001  # Fallback, but logged for debugging

def get_lp_token_value(issuer, amount_held, transactions):
    """Calculate LP token value with robust error handling."""
    try:
        amm_info = client.request(AMMInfo(amm_account=issuer)).result["amm"]
        required_fields = ["lp_token", "amount", "amount2"]
        if not all(field in amm_info for field in required_fields):
            logger.error(f"Missing required AMM fields for {issuer}")
            return 0
        ...
        if token_price <= 0:
            logger.error(f"Invalid token price for {token_currency}-{token_issuer}: {token_price}")
            return 0
        ...
        lp_value = amount_held * value_per_lp
        logger.debug(f"LP value for {issuer}: {lp_value} (pool={total_pool_value}, per_lp={value_per_lp})")
        return lp_value
    except Exception as e:
...
def get_token_pnl():
    ...
    try:
        ...
        for token, amount_held in holdings.items():
            ...
            unrealized_pnl = current_value - cost_basis if current_value and cost_basis else 0
